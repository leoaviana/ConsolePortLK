...
function Pet:Update()
	...
	for i, petActionButton in ipairs(self.Buttons) do 
		...
		if ( not isToken ) then
		    SetPortraitToTexture(petActionButton.icon, texture or [[Interface\AddOns\ConsolePortBar\Textures\ability-empty]]) 
			petActionButton.tooltipName = name
		else 
			SetPortraitToTexture(petActionButton.icon, _G[texture] or [[Interface\AddOns\ConsolePortBar\Textures\ability-empty]]) 
	...
	for button, keyID in pairs(actionButtons) do
		...
		Pet:WrapScript(button, 'PreClick', [[
			...
			if button == 'LeftButton' or button == 'RightButton' then
				...
				control:Run(OnKey, self:GetAttribute('keyID'), down) --barrun
			end
		]])
	end
